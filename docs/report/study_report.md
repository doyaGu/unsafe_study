...
- Full log: `evidence/miri/httparse.log`
...
**Full log**: `evidence/miri/serde_json.log`
...
**Full log**: `evidence/miri/bstr.log`
...
**All tasks complete.** See `docs/report/final_report.md` for the full case study report.
...
|------|--------|----------|
| **G1**: Hotspot map | [OK] Done | Geiger scans + per-crate annotations in `evidence/geiger/*_annotations.md` |
| **G2**: Run Miri + fuzzing | [OK] Done | 2 Miri findings (false positives), 0 fuzz crashes in 422M iterations |
| **G3**: Cross-tool comparison | [OK] Done | Full comparison in `docs/report/final_report.md` Section 5 |
...
routed through targeted `miri_harnesses` tests to avoid depending on
...
- New hotspot notes were written for the full intake batch in
  `evidence/geiger/*_annotations.md`.
