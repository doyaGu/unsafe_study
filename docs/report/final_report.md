...
| **Tier 2 (targeted)** | `miri_harnesses` or targeted smoke/matrix cases | Crate-local harness groups with longer budgets | memchr, winnow, toml_parser, simd-json, quick-xml, goblin, toml_edit, pulldown-cmark, roxmltree |
...
Full annotations are in `evidence/geiger/*_annotations.md`. Below is a summary of the top unsafe modules per crate.
...
**Log**: `evidence/miri/serde_json.log` (original), `evidence/miri/serde_json_no_symcheck.log` (clean re-run)
...
**Log**: `evidence/miri/bstr.log` (original), `evidence/miri/bstr_no_symcheck.log` (clean re-run)
...
1. **Coverage tier disparity.** Tier 1 crates (baseline) received full
   ...
   received only API smoke tests through `miri_harnesses` (Section 2.5).
...
| `evidence/miri/*.log` | Miri output logs (regenerate locally via `run_all.sh`) |
| `evidence/miri/simd_json_triage.md` | Focused Miri triage for simd-json |
| `docs/report/simd_json_stacked_borrows_explainer.md` | Technical explainer for simd-json Stacked Borrows finding |
| `evidence/fuzz/findings/*.log` | Fuzzing output logs (regenerate locally via `run_all.sh`) |
| `evidence/fuzz/corpus/` | Seed inputs for all fuzz targets |
...
|------|--------|----------|
| **G1**: Hotspot map | [OK] Complete | Geiger scans + per-crate annotations in `evidence/geiger/*_annotations.md` |
...
- **Miri scope**: `cargo miri test --offline --test api_smoke` in a dedicated
  local crate (`miri_harnesses`) with
  `MIRIFLAGS="-Zmiri-symbolic-alignment-check -Zmiri-strict-provenance"`.
...
- Batch pipeline summary:
  `docs/report/new_targets_pipeline.md`
- Focused finding write-up:
  `docs/report/new_target_findings.md`
...
- Hotspot notes (extension batch):
  `evidence/geiger/simd-json_annotations.md`,
  `evidence/geiger/goblin_annotations.md`,
  `evidence/geiger/quick-xml_annotations.md`,
  `evidence/geiger/toml_edit_annotations.md`,
  `evidence/geiger/pulldown-cmark_annotations.md`,
  `evidence/geiger/roxmltree_annotations.md`
- Hotspot notes (baseline, referenced in Section 3):
  `evidence/geiger/httparse_annotations.md`,
  `evidence/geiger/serde_json_annotations.md`,
  `evidence/geiger/bstr_annotations.md`
