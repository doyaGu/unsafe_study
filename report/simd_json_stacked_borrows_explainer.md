...
```rust
// Historical code (pre-fix) -- from miri_reports/simd_json_parse_str_tuple_fail.log
let first = unsafe {
...
draft exists at `report/simd_json_upstream_issue_draft.md` but has not been
submitted.
...
| Claim | Evidence |
|-------|---------|
| Pre-fix code called `input.as_mut_ptr()` per parse | `miri_reports/simd_json_parse_str_tuple_fail.log` lines 16, 21 |
...
| First string fails, last passes | Triage matrix (14 test cases) in `miri_reports/simd_json_triage.md` |
...
| Fix makes all Miri tests pass | `miri_reports/simd_json_miri_triage_after_api_refactor.log` |
